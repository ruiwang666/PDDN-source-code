...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ff = MLPBlock(hidden_size=num_channels+32, mlp_dim=num_channels * 4, act="GEGLU", dropout_rate=dropout)
        ...
        self.norm2 = nn.LayerNorm(num_channels+32)
        self.norm3 = nn.LayerNorm(num_channels+32)
...
#        x = torch.concat((self.norm1(x),y), axis=-1)
        ...
        if self.spatial_dims == 3:
            x = x.reshape(batch, height, width, depth, inner_dim+32).permute(0, 4, 1, 2, 3).contiguous()
