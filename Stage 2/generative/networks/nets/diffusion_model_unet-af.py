...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ff = MLPBlock(hidden_size=num_channels+3, mlp_dim=num_channels * 4, act="GEGLU", dropout_rate=dropout)
        ...
        self.norm2 = nn.LayerNorm(num_channels+3)
        self.norm3 = nn.LayerNorm(num_channels+3)
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.norm2 = norm_layer(in_channels+context_dim)
        mlp_hidden_dim = int((in_channels+context_dim) * mlp_ratio)
        self.pre_proj = nn.Linear(in_channels+context_dim, in_channels+context_dim)
        self.filter = AFNO3D(in_channels+context_dim, num_blocks, sparsity_threshold,
            hard_thresholding_fraction)
        self.mlp = Mlp(
            in_features=in_channels+context_dim,
...
#            context = context
#        elif x.shape[-1]==32:
#            context = F.avg_pool3d(context, (2,2,2))
#        elif x.shape[-1]==16:
#            context = F.avg_pool3d(context, (2,2,2))
#            context = F.avg_pool3d(context, (2,2,2))
#
...
#        if self.spatial_dims == 3:
#            x = x.reshape(batch, height, width, depth, inner_dim+3).permute(0, 4, 1, 2, 3).contiguous()
#
#        x = self.proj_out(x)
        return x
...
class Downsample(nn.Module):
    ...
    def __init__(
        ...
        if use_conv:
            ...
            self.op = Pool[Pool.AVG, spatial_dims](kernel_size=2, stride=(1, 2, 2))
...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        # input
        self.conv_conditional = Convolution(
            spatial_dims=spatial_dims,
            in_channels=in_channels,
            out_channels=32,
            strides=1,
    ...
    def forward(
        ...
        context = self.conv_conditional(context)
