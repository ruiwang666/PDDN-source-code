...
#        if mode == "concat":
        noisy_image = torch.cat([noisy_image, condition[:, :16]], dim=1)
#            condition = None
        diffusion_model = (
        ...
        prediction = diffusion_model(x=noisy_image, timesteps=timesteps, context=condition[:, 16:])
...
#            if mode == "concat":
            model_input = torch.cat([image, conditioning[:, :16]], dim=1)
            model_output = diffusion_model(
                model_input, timesteps=torch.Tensor((t,)).to(input_noise.device), context=conditioning[:, 16:]
            )
#            else:
#                model_output = diffusion_model(
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            condition_previous_gt = autoencoder_model_radar.encode(condition[:, :1]) * self.scale_factor
            condition_mask = autoencoder_model_radar.encode(condition[:, 1:2]) * self.scale_factor
            
            condition_wrf = autoencoder_model_wrf.encode(condition[:, 2:]) * self.scale_factor
            
#        print(condition_mask.shape)
#        print(condition_previous_gt.shape)
#        print(condition_wrf.shape)
            
        condition = torch.cat((condition_mask, condition_previous_gt, condition_wrf), 1)
...
#            )
        ...
        with torch.no_grad():
#            latent = autoencoder_model_radar.encode(inputs_) * self.scale_factor
            
            condition_previous_gt = autoencoder_model_radar.encode(conditioning[:, :1]) * self.scale_factor
            condition_mask = autoencoder_model_radar.encode(conditioning[:, 1:2]) * self.scale_factor
            
            condition_wrf = autoencoder_model_wrf.encode(conditioning[:, 2:]) * self.scale_factor
            
        conditioning = torch.cat((condition_mask, condition_previous_gt, condition_wrf), 1)
