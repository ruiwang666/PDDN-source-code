...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        """
        with torch.no_grad():
            latent = autoencoder_model.encode(inputs) * self.scale_factor
            condition_previous_gt = autoencoder_model.encode(condition[:, :1]) * self.scale_factor
            condition_mask = autoencoder_model.encode(condition[:, 1:]) * self.scale_factor
            
        condition = torch.cat((condition_previous_gt, condition_mask), 1)
...
#            )
        ...
        with torch.no_grad():
#            conditioning = autoencoder_model.encode(conditioning) * self.scale_factor
            condition_previous_gt = autoencoder_model.encode(conditioning[:, :1]) * self.scale_factor
            condition_mask = autoencoder_model.encode(conditioning[:, 1:]) * self.scale_factor
            
        conditioning = torch.cat((condition_previous_gt, condition_mask), 1)
        if isinstance(diffusion_model, SPADEDiffusionModelUNet):
...
#            decode = partial(autoencoder_model.decode_stage_2_outputs, seg=seg)
        image = autoencoder_model.decode(latent / self.scale_factor)
