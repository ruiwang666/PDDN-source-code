...
from autoencoder_2d.datasets.sevir.sevir_torch_wrap import SEVIRLightningDataModule
from autoencoder_2d.datasets.sevir.visualization import vis_sevir_seq
from autoencoder_2d.taming import AutoencoderKL, LPIPSWithDiscriminator
from autoencoder_2d.utils.optim import warmup_lambda

from autoencoder_2d.utils.download import (
    download_pretrained_weights,
    pretrained_sevirlr_vae_name)
from autoencoder_2d.utils.path import default_pretrained_vae_dir
from autoencoder_2d.utils.path import default_exps_dir
...
class RadarDataModule(LightningDataModule):
    def __init__(self, num_workers, batch_size=32):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        radar_data_train = torch.tensor(radar_data_train, dtype=torch.float32)
        
        # load radar val data
        radar_data_val = np.load('')
        radar_data_val = torch.tensor(radar_data_val, dtype=torch.float32)

        # load radar test data
        radar_data_test = np.load('')
        radar_data_test = torch.tensor(radar_data_test, dtype=torch.float32)

        if stage == 'fit' or stage is None:
            self.train_dataset = CustomDataset(radar_data_train)
            self.val_dataset = CustomDataset(radar_data_val)

        if stage == 'test' or stage is None:
            self.test_dataset = CustomDataset(radar_data_test)

    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, pin_memory=True, num_workers=self.num_workers, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, num_workers=self.num_workers, pin_memory=True, batch_size=self.batch_size)

    def test_dataloader(self):
        return DataLoader(self.test_dataset, num_workers=self.num_workers, pin_memory=True, batch_size=self.batch_size)
    @property
...
pytorch_state_dict_name = ''
pytorch_loss_state_dict_name = ''
...
def setup_autoenc_training(model_dir=''):

    num_gpus = torch.cuda.device_count()
    accelerator = "gpu" if (num_gpus > 0) else "cpu"
    devices = torch.cuda.device_count() if (accelerator == "gpu") else 1

    early_stopping = pl2.callbacks.EarlyStopping(
        "loss", patience=50, verbose=True
    )
    checkpoint = pl2.callbacks.ModelCheckpoint(
        dirpath=model_dir,
    ...
    trainer = pl2.Trainer(
        ...
        devices=devices,
        max_epochs=3000,
        strategy='ddp_find_unused_parameters_true' if (num_gpus > 1) else 'ddp_find_unused_parameters_true',
        callbacks=callbacks
    )
...
def main():
    ...
    dm = RadarDataModule(num_workers = 32, batch_size=micro_batch_size)
    dm.prepare_data()
    dm.setup()
